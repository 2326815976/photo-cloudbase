...
var mediaPipeNoisePatterns = [ "INFO: Created TensorFlow Lite XNNPACK delegate for CPU.", "Feedback manager requires a model with a single signature inference." ];

function wrapFilteredErr(writer) {
  return text => {
    if (typeof text === "string" && mediaPipeNoisePatterns.some(pattern => text.includes(pattern))) {
      return;
    }
    writer(text);
  };
...
// but before the wasm module is created.
{
  ...
  if (Module["printErr"]) err = wrapFilteredErr(Module["printErr"]);
