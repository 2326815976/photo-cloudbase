...
-- 表：待删除文件队列 (用于绕过 Edge Function 超时限制)
-- [🔥 核心逻辑] 业务表删除图片记录后，触发器会自动往这里写一条。
-- 你需要配置一个 Cron Job (定时任务) 每天调用 Edge Function 读取此表并执行 storage.remove()。
...
-- [新增] 表：约拍类型配置
-- [💻 前端对接] 预约页面的下拉菜单请从这里读取数据 (WHERE is_active = true)。
create table if not exists public.booking_types (
  id int generated by default as identity primary key,
  name text not null, -- 例如: '互勉', 'JK制服', '婚礼跟拍'
  is_active boolean default true, -- 管理员可软删除
  created_at timestamptz default now()
);
-- 预设数据
insert into public.booking_types (name) values ('互勉'), ('常规约拍'), ('婚礼跟拍'), ('活动记录');
  ...
  if new.status = 'cancelled' and old.status != 'cancelled' then
    ...
    if (now() at time zone 'Asia/Shanghai')::date >= old.booking_date then
      ...
      if is_admin is not true then
        raise exception '预约日期当天已无法自行取消，请联系摄影师。';
      end if;
    end if;
  end if;
  return new;
